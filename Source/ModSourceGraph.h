...
class ModSourceGraph : public juce::Component {
    private:
        ...
        ModSource* source = nullptr;   // points to either lfo or envelope
    ...
    public:
        ModSourceGraph(IgnitiveAudioProcessor& ap, ModSource* s) : audioProcessor(ap), source(s) {}

        void setSource(ModSource* s) {
            source = s;
            values.clear();
        }
